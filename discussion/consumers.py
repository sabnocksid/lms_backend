...
class DiscussionConsumer(AsyncWebsocketConsumer):
    ...
    def save_post(self, thread, user, content, parent_id=None):
        ...
        profile_image_url = self._get_profile_image_url_sync(user)

        return {
            "id": post.id,
            "creator": getattr(user, "full_name", user.username),
            "creator_id": user.id,
    ...
    def get_thread_messages(self, thread):
        ...
        for post in posts:
            ...
            profile_image_url = self._get_profile_image_url_sync(creator)

            messages.append({
                "id": post.id,
                "creator": getattr(creator, "full_name", creator.username),
    ...
    def _get_profile_image_url_sync(self, user):
        try:
            profile = getattr(user, "profile", None)
            if profile and profile.profile_image:
                return get_presigned_url(profile.profile_image, request=None)
        except Exception as e:
            logger.warning(f"Profile image fetch failed for user {user.id}: {e}")
        return None
